...
const configurationsKey = (orgId: string) => ['system-configurations', orgId] as const;
...
export const useSystemConfiguration = () => {
  ...
  const query = useQuery({
    queryKey: configurationsKey(orgId || 'none'),
  ...
  const loadConfigurations = async () => {
    await queryClient.refetchQueries({ queryKey: configurationsKey(orgId || 'none') });
  };
